...
def setUpRestaurantTable(data, cur, conn):
    ...
    cur.execute("CREATE TABLE Restaurants(restaurant_id TEXT PRIMARY KEY , name TEXT, address TEXT, zip_code TEXT, category_id INTEGER, rating REAL, review_count INTEGER, price TEXT)")
    conn.commit()
    
    restaurant_list = []
    for cell in data ['businesses']: # Iterate through the JSON data to get a list of restaurants
        #try:
        restaurant_id = cell['id']
        place_name = cell['name']
        location_address = cell['location']['address1']
        location_zip_code = cell['location']['zip_code']
        rating = float(cell['rating'])
        review_count = int(cell['review_count'])

        title = cell['categories'][0]['title']
        cur.execute('SELECT ID FROM Categories WHERE title = ?', (title,))
        ...
        cur.execute("INSERT INTO Restaurants(restaurant_id, name, address, zip_code, category_id, rating, review_count, price) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (restaurant_id, place_name, location_address, location_zip_code, category_id, rating, review_count, price))
        conn.commit()
...
def getRestaurantsByZip(zip_code, cur, conn):
    ...
    tuple_list = []
    cur.execute('SELECT name, address, zip_code, FROM Restaurant WHERE zip_code = ?;', (zip_code,))
    lines = cur.fetchall()
    ...
    return tuple_list # and returns a list of tuples. Each tuple contains 
...
def getRestaurantsByZipcodeAboveRatingAndByPrice(zip_code, rating, price, cur, conn):
    ...
    second_tuple = []
    cur.execute('SELECT name, address, rating, price FROM Restaurants WHERE zip_code = ? AND rating >= ? AND price = ?', (zip_code, rating, price))
    lines = cur.fetchall()

    for line in lines:
        second_tuple.append(line)
    ...
    return second_tuple # and of a particular price and returns a list of tuples.
...
def getRestaurantsAboveRatingAboveReviewsOfCategory(rating, review_count, category, cur, conn):
    ...
    third_tuple = []
    cur.execute('SELECT name, address, rating, review_count FROM Restaurants JOIN Categories ON Restaurants.category_id = Categories.id WHERE Categories.title = ? AND Restaurants.rating >= ? AND Restaurant.review_count >= ?', (category, rating, review_count))
    lines = cur.fetchall()

    for line in lines:
        third_tuple.append(line)


    conn.commit()
    
    # Note: You have to use JOIN for this task.
    return third_tuple # It returns a list of tuples, each tuple containing the
...
def main():
    ...
    #test_businesses_table(self)
    #test_restaurants_by_zip(self):
    #test_restaurants_by_zipcode_above_rating_by_price(self):
    #test_restaurants_above_rating_above_reviews_of_category(self):
